...
$(function () {
	...
	$(document).on("change", ".product_id", function () {
		...
		$.ajax({
			...
			type: 'GET',
			// ルーティングで設定したURL
			url: '/shinzemi/sales/' + id + '/get_product_price',
		}).done(function (results) {//成功
			var result = Number(results);
			if (!result == 0) {
				$('.job_description').eq(product_id_index).val(result);
			} else {
				$('.job_description').eq(product_id_index).val('');
			}
		}).fail(function (jqXHR, textStatus, errorThrown, results) {//失敗
			console.log(jqXHR, textStatus, errorThrown, results);
		}).always(function () {
			// 成否に関わらず実行されるコールバック
		});
	})
