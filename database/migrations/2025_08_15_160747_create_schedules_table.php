...
class CreateSchedulesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('schedules', function (Blueprint $table) {
            ...
            $table->unsignedInteger('school_building_id')->comment('校舎ID');
            $table->unsignedInteger('created_by')->comment('作成者ID');
            $table->enum('status', ['pending', 'approved', 'rejected'])->default('pending')->comment('承認状況');
            $table->unsignedInteger('approved_by')->nullable()->comment('承認者ID');
