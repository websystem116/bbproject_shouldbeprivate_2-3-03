...
@section('content')
    <div class="py-12 max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <form action="{{ route('invoice.index') }}" name="mainform" method="GET">
            <div class="flex rounded items-center p-2">
                <input type="hidden" name="student_no" value="{{ request()->query('student_no') }}">
                <select name="search_charge_month" class="w-3/4 rounded focus:outline-none text-gray-700">
                    <option value="">全期間</option>
                    @foreach ($invoices['uniqueChargeMonths'] as $month)
                        <option value="{{ $month }}" @if ($search_charge_month == $month) selected @endif>
                        ...
                        <tr class="bg-white border-b">
                            <td class="px-6 py-4 relative">
                                <div class='text-lg font-bold'>
                                    {{ date('Y年m月', strtotime($invoice->charge_month . '-01')) }}請求書
                                </div>
                                <div class=''>
                                    生徒番号：{{ $invoice->student_no }}
                                </div>
                                <div class=''>
                                    更新日時：{{ $invoice->updated_at->format('Y年m月d日') }}
                                </div>
                                <div class=''>
                                    公開期限：{{ $invoice->created_at->addMonths(3)->format('Y年m月d日') }}
                                </div>
                                <!-- ボタンを追加 -->
        ...
        {{ $invoices['invoices']->appends($param)->links() }}
    </div>
@endsection
