        ...
        <div class="col-md-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3 class="panel-title">お知らせ編集</h3>
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route('announcements.update', $announcement->id) }}">
                                ...
                                <div class="checkbox">
                                    <label>
                                        <input type="checkbox" 
                                               ...
                                               value="all" 
                                               {{ in_array('all', old('distribution_targets', $announcement->distribution_targets ?? [])) ? 'checked' : '' }}
                                               id="select_all">
                                            ...
                                            <div class="checkbox">
                                                <label>
                                                    <input type="checkbox" 
                                                           ...
                                                           {{ in_array($schoolBuilding->id, old('distribution_targets', $announcement->distribution_targets ?? [])) ? 'checked' : '' }}>
                                                    {{ $schoolBuilding->name }}
                                                    @if($schoolBuilding->name_short)
                                                        <span class="label label-info">{{ $schoolBuilding->name_short }}</span>
                                                    @endif
                                                </label>
                                            </div>
                                        </div>
                        ...
                        @if($announcement->created_at != $announcement->updated_at)
                            <div class="alert alert-info">
                                <small>
                                    ...
                                    最終更新: {{ $announcement->updated_at->format('Y年m月d日 H:i') }}
                                </small>
                            </div>
                        @endif
                        ...
                        <div class="form-group">
                            ...
                            @else
                                <button type="submit" name="action" value="save_draft" class="btn btn-primary" onclick="tinyMCE.triggerSave();">
                                    <span class="glyphicon glyphicon-floppy-disk"></span> 更新
                                </button>
                            @endif
                            <a href="{{ route('announcements.show', $announcement->id) }}" class="btn btn-info">
                                <span class="glyphicon glyphicon-eye-open"></span> 詳細を見る
                            </a>
                            <a href="{{ route('announcements.index') }}" class="btn btn-default">
                                <span class="glyphicon glyphicon-arrow-left"></span> 一覧に戻る
                            </a>
                        </div>
                    </form>
                </div>
...
function showMobilePreview() {
    ...
    var previewHtml = `
        <div style="display:flex; background-color: #f8f9fa; border-bottom: 2px solid #e9ecef; padding: 15px 0;">
            <h1 style="color: black; font-size: 20px; margin: 0; border-bottom: 4px double black; padding-bottom: 4px;">${title}</h1>
        </div>
        ...
        <div style="margin-top: 30px;">
            <div style="background-color: #6c757d; color: white; padding: 12px; text-align: center; text-decoration: none; display: block;">
                お知らせ一覧に戻る
            </div>
        </div>
    `;
