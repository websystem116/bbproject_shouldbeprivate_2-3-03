...
for test_image_url in test_image_url_array:
  ...
  if 'id' in image_name:#アンケードIDを識別
    test_teacher_label = check_ID( test_image_url )
    # retval.append([ image_name,test_teacher_label ])  # アンケートIDの値
    retval[image_name] = str(test_teacher_label)  # アンケートIDの値
    ...
    with chainer.using_config('train', False), chainer.using_config('enable_backprop', False):
      ...
      retval[image_name] = str(test_teacher_label)  # レベルが値だっぺ

  elif 'alphabet' in image_name:#ABCDE判別 ファイル名にclassってはいってるかどうか
    ...
    with chainer.using_config('train', False), chainer.using_config('enable_backprop', False):
      ...
      if test_teacher_label == 0:
        ...
        # retval.append([ image_name,'1' ]) # 1にレ点あり
        retval[image_name] = '1' # 1にレ点あり
      elif test_teacher_label == 2:
        # retval.append([ image_name,'2' ]) # 2にレ点あり
        retval[image_name] = '2' # 2にレ点あり
      elif test_teacher_label == 3:
        # retval.append([ image_name,'3' ]) # 3にレ点あり
        retval[image_name] = '3' # 3にレ点あり
      elif test_teacher_label == 4:
        # retval.append([ image_name,'4' ]) # 4にレ点あり
        retval[image_name] = '4'# 4にレ点あり
      else :
        # retval.append([ image_name,'' ]) #  #''
        retval[image_name] = '0' #  #''
...
retval2 = json.dumps(retval, ensure_ascii=False, indent=2)
print(retval2)
